%THIS FUMCTION ILLUSTRATE HOW TO USE OWN PROGRAM
function [W] = amuse(X)
...
%
  ...
  RRR=real(sqrt(1./Dx(1:n)));TTT=sqrt(1./Dx(1:n));
   Q= diag(real(sqrt(1./Dx(1:n))))*Ux(:,1:n)';
end;
%
...
% for example -1 or 2.
N=max(size(Xb));qqqq=kron(mean(Xb')',ones(1,N));
Xb=Xb-kron(mean(Xb')',ones(1,N));

Rxbxbp=(Xb(:,1:N-1)*Xb(:,2:N)')/(N-1);
Rxbxbp= Rxbxbp+Rxbxbp';
[Vxb Dxb]=eig(Rxbxbp);TTTTTTT=diag(Dxb);
